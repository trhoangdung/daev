...
def get_benchmark():
    ...
    # PEEC
    contents = loadmat('MNA_1.mat')
    E = contents['E']
    A = contents['A']
    B = contents['B']
    n = A.shape[0]
    C = np.zeros((1, n))
    C[0, 0] = 1    # output is x_1(t)
    print "\n########################################################"
    print "\nMODIFIED NODAL ANLASIS-1:"
    print "\ndimensions: {}".format(E.shape[0])
    print "\nnumber of input = {}".format(B.shape[1])
    print "\nnumber of output = {}".format(C.shape[0])
    print "\nE = {} \nA ={} \nB={} \nC={}".format(E, A, B, C)

    return csc_matrix(E), csc_matrix(A), csc_matrix(B), csc_matrix(C)
...
def convert_to_auto_dae(dae_sys):
    ...
    m = dae_sys.matrix_b.shape[1]
    u_mat = np.zeros((m, m))    # user-defined input
    u_mat[7, 8] = -1
    u_mat[0, 7] = 1     # a since inputs
...
def construct_init_set(basic_matrix):
    ...
    init_set_basic_matrix = basic_matrix[:, 0:4]
    print "\ninit_set_basic_matrix shape = {}".format(init_set_basic_matrix.shape)

    alpha_min = np.array([[2.1], [1.0], [0.0], [1.2]])
    alpha_max = np.array([[2.5], [1.2], [0.4], [1.5]])
...
def plot_vline_set(list_of_line_set_list, totime, num_steps):
    ...
    ax1.legend([r'$x_{1}(t)$'], fontsize=20)
...
def plot_unsafe_trace(veri_result):
    ...
    ax1.legend(['Ouput: -$x_{1}(t)$', 'US: Unsafe boundary'], fontsize=20)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.xlabel('$t$ (seconds)', fontsize=20)
    plt.ylabel(r'-$x_{1}(t), US$', fontsize=25)
