...
def get_benchmark():
    ...
    # PEEC
    contents = loadmat('MNA_4.mat')
    E = contents['E']
    A = contents['A']
    B = contents['B']
    n = A.shape[0]
    C = np.zeros((1, n))
    C[0, 1] = 1    # output is x_2(t)
    print "\n########################################################"
    print "\nMODIFIED NODAL ANLASIS-4:"
...
def construct_init_set(basic_matrix):
    ...
    init_set_basic_matrix = basic_matrix[:, 0:3]
    print "\ninit_set_basic_matrix shape = {}".format(init_set_basic_matrix.shape)

    alpha_min = np.array([[1.0], [0.8], [0.0]])
    alpha_max = np.array([[1.5], [1.2], [0.6]])
